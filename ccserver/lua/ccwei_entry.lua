local ccwei_global=require "ccwei_global"
...
if "POST"==request_method then
    ...
    if body_str==nil then
        ccwei_global:returnwithcode(ccwei_global.ERR_NO_BODYDATA,nil)
    end
    
    local status,userdata=pcall(cjson.decode,body_str)
    if status == false then
        ccwei_global:returnwithcode(ccwei_global.ERR_INVALID_JSON_FORMAT,nil)
    end
    ...
    if userdata['uid'] == nil or userdata['devicetype'] ==nil or userdata['usertype'] ==nil or userdata['cmdid'] == nil or userdata['version'] == nil or userdata['time'] == nil then
        ccwei_global:returnwithcode(ccwei_global.ERR_INVALID_PARAM,nil)
    end
    ...
    if uid == nil or devicetype ==nil or usertype==nil or cmdid == nil or version == nil or reqtime == nil then
        ccwei_global:returnwithcode(ccwei_global.ERR_PARAM_TYPE,nil)
    end
    
    if devicetype ~=1101 and devicetype ~= 1102 then
        ccwei_global:returnwithcode(ccwei_global.ERR_UNSUPPORT_DEVICE,nil)
    end
    
    if usertype ~= 2 and usertype ~= 1 then
        ccwei_global:returnwithcode(ccwei_global.ERR_UNSUPPORT_USERTYPE,nil)
    end
    ...
    if version == "0.1" then
        ...
        elseif cmdid == 2 then
            local ccfei_uploadinfo_obj = require "ccfei_uploadinfo"
			local ccfei_uploadinfo = ccfei_uploadinfo_obj:new()
            ccfei_uploadinfo:process(userdata)
        elseif cmdid == 3 then
		...
		elseif cmdid == 7 then
            ...
            ccwei_global:returnwithcode(ccwei_global.ERR_UNSUPPORT_CMD,nil)
        end
    else
        ccwei_global:returnwithcode(ccwei_global.ERR_UNSUPPORT_VERSION,nil)
    end
    
else
    ccwei_global:returnwithcode(ccwei_global.ERR_INVALID_METHOD,nil)
end
