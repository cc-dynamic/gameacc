...
local MOD_ERR_BASE = fei_global.ERR_MOD_QUERYACCT_BASE
...
function _M.checkparam(self,userreq)
    ...
    if userreq['data']==nil then
        fei_global:returnwithcode(self.MOD_ERR_INVALID_PARAM,nil)
    end
    ...
    if timestart == nil or timeend==nil or uid==nil then
        fei_global:returnwithcode(self.MOD_ERR_PARAM_TYPE,nil)
    end
...
function _M.query_acct(self,db)
    ...
    if not res then
    	fei_global:returnwithcode(self.MOD_ERR_QUERY_ACCT,nil)
    end
...
function _M.process(self,userreq)
    ...
    fei_global:returnwithcode(0,acct_info)
end
